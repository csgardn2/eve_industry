...
namespace args
{
    ...
    const std::vector<std::string_view> eve_industry_t::mode_names_ =
    {
    ...
    const std::vector<std::string_view> eve_industry_t::mode_values_ =
    {
    ...
    std::string eve_industry_t::valid_mode_values() const
    {
        
        std::string ret("{");
        
        for (unsigned ix = 0, last_ix = eve_industry_t::mode_values_.size() - 1; ix <= last_ix; ix++)
        {
            ret += eve_industry_t::mode_values_[ix];
            if (ix == last_ix)
                ret += '}';
            else
                ret += ", ";
    ...
    void eve_industry_t::parse(unsigned argc, char const* const* argv)
    {
        ...
        {
            if (mode == eve_industry_t::mode_values_[ix])
            {
    ...
    void eve_industry_t::read_from_json_file(std::istream& file)
    {
    ...
    void eve_industry_t::read_from_json_buffer(std::string_view buffer)
    {
    ...
    void eve_industry_t::read_from_json_json(const Json::Value& json_root)
    {
    ...
    void eve_industry_t::write_to_json_file(std::ostream& file, unsigned indent_start, unsigned spaces_per_tab) const
    {
    ...
    void eve_industry_t::write_to_json_buffer(std::string& buffer, unsigned indent_start, unsigned spaces_per_tab) const
    {
        ...
        buffer += indent_1;
        buffer += "\"mode\": \"";
        buffer += eve_industry_t::mode_values_[unsigned(this->mode_)];
        buffer += "\",\n";
    ...
    std::istream& operator>>(std::istream& stream, eve_industry_t& destination)
    {
        try
        {
